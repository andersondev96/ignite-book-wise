...
import { LastReading } from '@/src/components/LastReading'
...
describe('<LastReading />', () => {
  ...
  it('renderiza card com última leitura corretamente', async () => {
    ;(useSession as any).mockReturnValue({
      data: mockSession,
      status: 'authenticated',
    })
    ;(api.get as Mock).mockImplementation(async (url: string) => {
      if (url.includes('/ratings/user-latest')) {
    ...
    await waitFor(() => {
      expect(screen.getByText(/sua última leitura/i)).toBeInTheDocument()
